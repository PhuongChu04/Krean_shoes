...
@section('content')
    ...
    <div class="flat-spacing-13">
        <div class="container">
            ...
            <div class="row">
                <div class="col-xl-8">
                    <form id="checkout-form" class="tf-checkout-cart-main" action="{{ route('client.checkout.process') }}"
                        method="POST">
                        @csrf
                        ...
                        <div class="box-ip-checkout">
                            ...
                            <fieldset class="tf-field style-2 style-3 mb_16">
                                <div class="tf-select select-square">
                                    ...
                                    <select id='address_id' name='address_id' required
                                        class="@error('address_id') is-invalid @enderror">
                            ...
                            <div class="mb_16">
                                <button type="button" class="tf-btn btn-outline-dark btn-sm" data-bs-toggle="modal"
                            ...
                            <fieldset class="tf-field style-2 style-3 mb_16">
                                <div class="tf-select select-square">
                                    <select id='voucher_id' name='voucher_id'
                                        class="@error('voucher_id') is-invalid @enderror">
                                        <option value="">Chọn voucher (không bắt buộc)</option>
                                        @foreach ($availableVouchers as $voucher)
                                            <option value="{{ $voucher->id }}"
                                                {{ old('voucher_id') == $voucher->id ? 'selected' : '' }}>
                                                {{ $voucher->code }} -
                                                {{ $voucher->type === 'percentage' ? $voucher->discount_amount . '%' : number_format($voucher->discount_amount, 0, ',', '.') . '₫' }}
                            ...
                            <fieldset>
                                <label class="check-ship">
                                    ...
                                    <span class="text text-sm">
                                        ...
                                        <small class="d-block text-muted mt-1">Thanh toán sau khi nhận và kiểm tra
                                            hàng.</small>
                                    </span>
                                </label>
                            </fieldset>
                            <fieldset class="mb_16">
                                <label class="check-ship">
                                    ...
                                    <span class="text text-sm">
                                        ...
                                        <small class="d-block text-muted mt-1">Hỗ trợ ATM nội địa, QR Code và Internet
                                            Banking.</small>
                                    </span>
                                </label>
                            </fieldset>
    ...
    <li class="order-item" style="display: flex; align-items: center; gap: 10px; padding: 8px 0; border-bottom: 1px solid #f0f0f0; list-style: none;">

        {{-- Ảnh + số lượng --}}
        <div style="position: relative; flex-shrink: 0;">
            <figure style="width: 60px; height: 60px; border-radius: 8px; overflow: hidden; margin: 0; border: 1px solid #eee;">
            ...
            <span style="position: absolute; top: -5px; right: -5px; background: #333; color: #fff; font-size: 10px; font-weight: 700; min-width: 18px; height: 18px; border-radius: 999px; display: flex; align-items: center; justify-content: center; padding: 0 4px;">
                {{ $item->quantity }}
            </span>
        </div>
        ...
        <div style="flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px;">
            <p class="name text-sm fw-medium" style="margin: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                {{ $item->productVariant->product->name }}
            </p>
            <span style="font-size: 11px; color: #999;">
                {{ $item->productVariant->attribute_name ?? 'Không có biến thể' }}
            </span>
            <span class="price text-sm fw-medium" style="color: #000000;">
...
@endforeach
                            ...
                            <ul class="list-total">
                                <li class="total-item text-sm d-flex justify-content-between">
                                    ...
                                    <span id="summary-subtotal"
                                        class="price-sub fw-medium">{{ number_format($subtotal, 0, ',', '.') }} ₫</span>
                                </li>
                                <li class="total-item text-sm d-flex justify-content-between">
                                    <span>Phí vận chuyển:</span>
                                    <span id="summary-shipping"
                                        class="price-discount fw-medium">{{ number_format($shipping, 0, ',', '.') }}
                                        ₫</span>
                                </li>
                                <li id="summary-discount-row" class="total-item text-sm d-flex justify-content-between"
                                    style="display: none;">
                                    <span>Giảm giá:</span>
                            ...
                            <div class="subtotal text-lg fw-medium d-flex justify-content-between">
                                ...
                                <span id="summary-total"
                                    class="total-price-order">{{ number_format($total, 0, ',', '.') }} ₫</span>
                            </div>
                            <div class="btn-order">
                ...
                <div class="modal-content">
                    ...
                    <form id="add-address-form" action="{{ route('client.addresses.store') }}" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="modal_name" class="form-label">Họ tên <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="modal_name" name="name" required>
                            </div>
                            <div class="mb-3">
                                <label for="modal_phone" class="form-label">Số điện thoại <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="modal_phone" name="phone" required>
                            </div>
                            <div class="mb-3">
                                <label for="modal_province" class="form-label">Tỉnh/Thành phố <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="mb-3">
                                <label for="modal_district" class="form-label">Quận/Huyện <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="mb-3">
                                <label for="modal_ward" class="form-label">Phường/Xã <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="mb-3">
                                <label for="modal_address" class="form-label">Địa chỉ chi tiết <span
                                        class="text-danger">*</span></label>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const loadDistricts = async (provinceCode, selectedDistrict = '', selectedWard = '') => {
                    ...
                    try {
                        ...
                        if (selectedDistrict) {
                            const selectedOption = Array.from(modalDistrict.options).find(option => option
                ...
                const loadProvinces = async () => {
                    ...
                    try {
                        ...
                        if (oldProvince) {
                            const selectedOption = Array.from(modalProvince.options).find(option => option
                                .value === oldProvince);
                ...
                if (modalProvince && modalDistrict && modalWard) {
                    modalProvince.addEventListener('change', async () => {
                        ...
                        console.log('Province selected:', {
                            ...
                            code: provinceCode,
                            options: Array.from(modalProvince.options).map(option => ({
                                value: option.value,
                                text: option.text,
                    ...
                    modalDistrict.addEventListener('change', async () => {
                        ...
                        console.log('District selected:', {
                            ...
                            text: selectedOption?.text,
                            code: districtCode,
                            options: Array.from(modalDistrict.options).map(option => ({
                                value: option.value,
                                text: option.text,
                                code: option.dataset.code
                            })),
                    ...
                    modalWard.addEventListener('change', () => {
                        ...
                        console.log('Ward selected:', {
                            ...
                            options: Array.from(modalWard.options).map(option => ({
                                value: option.value,
                                text: option.text,
                                code: option.dataset.code
                            })),
