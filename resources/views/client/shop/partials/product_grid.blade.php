    ...
    <div class="card-product style-center" data-product-id="{{ $product->id }}"
        ...
        <div class="card-product-wrapper">
            <a href="{{ route('client.product.detail', $product->slug ?? $product->id) }}" class="product-img">
                <img class="img-product lazyload" data-src="{{ $mainImage }}" src="{{ $mainImage }}"
                    alt="{{ $product->name }}">

                <img class="img-hover lazyload" data-src="{{ $hoverImage }}" src="{{ $hoverImage }}"
                    alt="{{ $product->name }}">
            </a>
                ...
                <li class="wishlist">
                    @auth
                        @if (in_array($product->id, $wishlistProductIds ?? []))
                            {{-- Đã yêu thích → nút XÓA --}}
                            <form action="{{ route('client.wishlist.destroy', $product->id) }}" method="POST"
                                class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="bg-surface hover-tooltip tooltip-left box-icon btn btn-link p-0 border-0 bg-transparent"
                                    style="color: #e53935;" title="Bỏ yêu thích">
                                    <span class="icon icon-heart2" style="color: #e53935;"></span>
                                    <span class="tooltip">Bỏ yêu thích</span>
                                </button>
                            </form>
                        @else
                            {{-- Chưa yêu thích → nút THÊM --}}
                            <form action="{{ route('client.wishlist.store', $product->id) }}" method="POST"
                                class="d-inline">
                                @csrf
                                <button type="submit"
                                    class="bg-surface hover-tooltip tooltip-left box-icon btn btn-link p-0 border-0 bg-transparent text-muted">
                    ...
                    @else
                        ...
                        <a href="{{ route('auth.login') }}"
                            class="bg-surface hover-tooltip tooltip-left box-icon text-muted">
                            <span class="icon icon-heart2"></span>
                            <span class="tooltip">Đăng nhập để yêu thích</span>
                        </a>
                    @endauth
                </li>
